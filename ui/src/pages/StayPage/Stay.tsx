...
function Stay() {
  ...
  useEffect(() => {
    axios
      .get(
        `${apiURL}/api/places-to-stay/list?stayType=${stayType}&region=${region}&page=${page}`
      )
      .then((response) => {
  ...
  return (
    <div>
      <PageHero
        imageSrc="https://ernestocapstoneimages.blob.core.windows.net/images/stay-hero.jpg"
        altText="resort hotel on island"
      />
      <div className="container">
        <PageIntro
          heading="places to stay"
          introText="Taniti has a wide variety of lodging to suit the needs of just about any traveler. Travelers on a budget and who don't
    ...
    several private condos and beach-front residences."
        ...
        <form className="row ps-5 mt-5">
          <Dropdown
            apiEndpoint="/api/places-to-stay/type"
            label="By Type"
            id="stayType"
            onChange={(newStayType) => setStayType(newStayType)}
          />
          <Dropdown
            apiEndpoint="/api/places-to-stay/region"
            label="Region"
            id="region"
